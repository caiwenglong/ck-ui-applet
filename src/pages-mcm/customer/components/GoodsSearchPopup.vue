<template>
  ...
  >
    <view class="goods_search_popup">
      <view class="goods_search_header">
        <text class="goods_search_title">品类搜索</text>
        <view class="goods_search_close" @click="handleClose">
          <text>✕</text>
        </view>
      </view>
      <view class="goods_search_content">
        <wd-input
          v-model="goodsSearchInput"
          clearable
          placeholder="请输入品类名称"
        ...
        <view v-if="recentSearches.length > 0" class="recent_searches">
          ...
          <view class="recent_searches_tags">
            <view
              ...
              :key="index"
              :class="{ active: goodsSearchInput === item }"
      ...
      <view class="goods_search_footer">
        <wd-button custom-class="goods_search_reset_btn" @click="handleReset">
          重置
        </wd-button>
        <wd-button custom-class="goods_search_submit_btn" type="primary" @click="handleSubmit">
          搜索
        </wd-button>
      </view>
    </view>
...
const STORAGE_KEY = 'customer_goods_recent_searches'
...
watch(
  ...
  (visible: any) => {
    if (visible) {
      goodsSearchInput.value = props.value || ''
      loadRecentSearches()
    }
  },
  { immediate: true },
...
watch(
  ...
  (newValue: any) => {
    if (!props.modelValue) {
      goodsSearchInput.value = newValue || ''
    }
  },
)
...
function selectRecentSearch(keyword: string) {
  goodsSearchInput.value = keyword
}
...
function handleSubmit() {
  const keyword = goodsSearchInput.value?.trim() || ''
  if (keyword) {
...
.goods_search_popup {
  ...
  .goods_search_footer {
    ...
    .goods_search_reset_btn {
      flex: 1;
    }

    .goods_search_submit_btn {
      flex: 1;
    }
  }
}
